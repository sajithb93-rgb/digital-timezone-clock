...
function ChartAnnotations({chart,series,host,candles,smc,elliott,mode,tick,layers}:{chart:any;series:any;host:HTMLElement|null;candles:Candle[];smc:any;elliott:any;mode:Mode;tick:number;layers:any}){
 ...
 return <div className="chart-overlay-wrap"><div className="analysis-debug">ANALYSIS · {candles.length} CLOSED CANDLES · SMC {smc.events.length} BOS/CHOCH · FVG {smc.fvgs.length} · OB {smc.orderBlocks.length} · LIQ {smc.liquidityHighs.length+smc.liquidityLows.length} · ELLIOTT {elliott.primary?"READY":elliott.correction?"ABC CANDIDATE":"—"}</div><svg className="chart-overlay" width={width} height={height} viewBox={`0 0 ${width} ${height}`}>{showSMC&&<>{zones}{structure}{liquidity}{trade}</>}{showElliott&&<>{eliteTrade}{wave}{fib}</>}</svg>{showSMC&&<div className={`setup-panel smc-setup-panel ${smc.setup.direction.toLowerCase()}`}><div className="setup-head"><span>SMC SETUP LEVELS</span><strong>{smc.setup.direction}</strong></div><div className="setup-grid"><span>Trend<b>{smc.trend}</b></span><span>Score<b>{smc.setup.confidence}/100</b></span><span>Entry<b>{smc.setup.entry!=null?smc.setup.entry.toFixed(4):"—"}</b></span><span>SL<b>{smc.stop!=null?smc.stop.toFixed(4):"—"}</b></span><span>TP1<b>{smc.targets?.[0]?.toFixed(4)||"—"}</b></span><span>TP2<b>{smc.targets?.[1]?.toFixed(4)||"—"}</b></span><span>TP3<b>{smc.targets?.[2]?.toFixed(4)||"—"}</b></span><span>R:R<b>{smc.setup.rr?smc.setup.rr.toFixed(2)+":1":"—"}</b></span></div></div>}{showElliott&&<div className={`setup-panel elitewave-setup-panel ${elite?.direction==="bearish"?"bear":""}`}><div className="setup-head"><span>ELITEWAVE SETUP LEVELS</span><strong>{elite?.direction?elite.direction.toUpperCase():"WAIT"}</strong></div>{elite&&elite.entry!=null&&elite.invalidation!=null?<div className="setup-grid"><span>Phase<b>{elliott.phase}</b></span><span>Quality<b>{elliott.confidence}/100</b></span><span>Entry<b>{elite.entry.toFixed(4)}</b></span><span>SL / INV<b>{elite.invalidation.toFixed(4)}</b></span><span>TP1<b>{elite.targets?.[0]?.toFixed(4)||"—"}</b></span><span>TP2<b>{elite.targets?.[1]?.toFixed(4)||"—"}</b></span><span>TP3<b>{elite.targets?.[2]?.toFixed(4)||"—"}</b></span><span>R:R<b>{elite.targets?.[0]!=null?(Math.abs(elite.targets[0]-elite.entry!)/Math.abs(elite.entry!-elite.invalidation)).toFixed(2)+":1":"—"}</b></span></div>:<div className="setup-empty">NO QUALIFIED 1–5 TRADE SETUP</div>}</div>} </div>;
}
