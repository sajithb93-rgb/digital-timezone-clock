...
export default function Home(){
 const [mode,setMode]=useState<Mode>("combined"),[symbol,setSymbol]=useState("BTCUSDT"),[interval,setInterval]=useState<(typeof intervals)[number]>("15m"),[marketType,setMarketType]=useState<MarketKind>("spot");
 ...
 return <main className={`app-shell theme-${theme}`}>
  ...
  <aside className="analysis-column">
   ...
   <details className="more-tools side-tools"><summary>More market data <span>MTF · Derivatives · Scanner · Status</span></summary><div className="panel-card mtf-card"><div className="section-title">MULTI-TIMEFRAME</div>{mtf.frames.map(f=><div className="mtf-row" key={f.interval}><span>{f.interval}</span><b className={f.trend==="Bullish"?"positive":f.trend==="Bearish"?"negative":""}>{f.available?f.trend:"UNAVAILABLE"}</b><small>{f.structure} · {f.score}/100</small></div>)}</div>
...
function ChartAnnotations({chart,series,host,candles,smc,elliott,mode,tick,layers}:{chart:any;series:any;host:HTMLElement|null;candles:Candle[];smc:any;elliott:any;mode:Mode;tick:number;layers:any}){
 ...
 const eliteTrade=layers.trade&&elite&&elite.entry!=null&&elite.invalidation!=null?<>{(()=>{const p2=elite.points.find((p:any)=>p.label==="2"),xe=p2?xOf(p2.index):null,ye=yOf(elite.entry);return ye==null?null:<g>{xe!=null&&<circle cx={xe} cy={ye} r="5" className="elite-entry-marker"/>}<line x1={x0} x2={xLast} y1={ye} y2={ye} className="elite-entry-line"/>{text(xLast-120,ye,"EW ENTRY "+elite.entry.toFixed(4),"elite-entry-label")}</g>})()}{(()=>{const y=yOf(elite.invalidation);return y==null?null:<g><line x1={x0} x2={xLast} y1={y} y2={y} className="elite-sl-line"/>{text(xLast-100,y,"EW SL "+elite.invalidation.toFixed(4),"elite-sl-label")}</g>})()}{(elite.targets||[]).slice(0,3).map((p:number,i:number)=>{const y=yOf(p);return y==null?null:<g key={"ewtp"+i}><line x1={x0} x2={xLast} y1={y} y2={y} className="elite-tp-line"/>{text(xLast-60,y,"EW TP"+(i+1),"elite-tp-label")}</g>})}</>:null;
 const drawWave=(points:any[],keyPrefix:string,labelClass="wave-label")=><>{points.map((p:any,i:number)=>{const x=xOf(p.index),y=yOf(p.price),n=points[i+1],nx=n?xOf(n.index):null,ny=n?yOf(n.price):null;return x==null||y==null?null:<g key={keyPrefix+i}>{nx!=null&&ny!=null&&<line x1={x} y1={y} x2={nx} y2={ny} className="wave-line"/>}{text(x,y,p.label,labelClass)}</g>})}</>;
 const wave=<>{
  elliott.primary&&drawWave(elliott.primary.points,"impulse-")
 }{elliott.correction&&drawWave(elliott.correction.points.slice(0,3),"abc-","wave-abc-label")}</>;
 return <div className="chart-overlay-wrap"><div className="elitewave-setup-strip">{elite&&elite.entry!=null&&elite.invalidation!=null?<><span>ELITEWAVE</span><b>{elite.direction.toUpperCase()}</b><span>ENTRY <strong>{elite.entry.toFixed(4)}</strong></span><span>SL <strong>{elite.invalidation.toFixed(4)}</strong></span>{(elite.targets||[]).slice(0,3).map((p:number,i:number)=><span key={"ews"+i}>TP{i+1} <strong>{p.toFixed(4)}</strong></span>)}</>:<span>ELITEWAVE · NO QUALIFIED 1–5 TRADE SETUP</span>}</div><div className="analysis-debug">ANALYSIS · {candles.length} CANDLES · SMC {smc.events.length} BOS/CHOCH · FVG {smc.fvgs.length} · OB {smc.orderBlocks.length} · LIQ {smc.liquidityHighs.length+smc.liquidityLows.length} · ELLIOTT {elliott.primary?"READY":elliott.correction?"ABC CANDIDATE":"—"}</div><svg className="chart-overlay" width={width} height={height} viewBox={`0 0 ${width} ${height}`}>{zones}{structure}{liquidity}{trade}{eliteTrade}{wave}</svg>{<div className={`setup-panel ${smc.setup.direction.toLowerCase()}`}><div className="setup-head"><span>SMC SETUP</span><strong>{smc.setup.direction}</strong></div><div className="setup-grid"><span>Trend<b>{smc.trend}</b></span><span>Score<b>{smc.setup.confidence}/100</b></span><span>Entry<b>{smc.setup.entry!=null?smc.setup.entry.toFixed(4):"—"}</b></span><span>SL<b>{smc.stop!=null?smc.stop.toFixed(4):"—"}</b></span><span>TP1<b>{smc.targets?.[0]?.toFixed(4)||"—"}</b></span><span>TP2<b>{smc.targets?.[1]?.toFixed(4)||"—"}</b></span><span>TP3<b>{smc.targets?.[2]?.toFixed(4)||"—"}</b></span><span>R:R<b>{smc.setup.rr?smc.setup.rr.toFixed(2)+":1":"—"}</b></span></div></div>}</div>;
}
