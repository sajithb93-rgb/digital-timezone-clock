...
export function flowSnapshot(c:Candle[]):FlowSnapshot{
 ...
 if(!data.length)return{buyVolume:0,sellVolume:0,delta:0,deltaRatio:0,cumulativeDelta:0,volumeRatio:0,pressure:"BALANCED"};
 ...
 return{
  ...
  deltaRatio:ratio,
  cumulativeDelta:cum,
  volumeRatio:last.volume/Math.max(base,1e-12),
  pressure:ratio>.08?"BUYERS":ratio<-.08?"SELLERS":"BALANCED"
 };
}

export function detectRegime(c:Candle[]):Regime{
 const data=c.at(-1)?.closed===false?c.slice(0,-1):c;
 if(data.length<20)return{regime:"TRANSITION",strength:0,atr:0,rangePercent:0};
 const n=14;
 const tr=data.slice(-n).map((x,i,a)=>i===0?x.high-x.low:Math.max(x.high-x.low,Math.abs(x.high-a[i-1].close),Math.abs(x.low-a[i-1].close)));
 const atr=tr.reduce((a,b)=>a+b,0)/tr.length;
 const q=data.slice(-60),hi=Math.max(...q.map(x=>x.high)),lo=Math.min(...q.map(x=>x.low));
...
export function confluence(s:any,flow:FlowSnapshot,regime:Regime):ConfluenceBreakdown{
 ...
 const recent=(index:number,bars=20)=>index>=Math.max(0,lastIndex-bars);
 const setupDir=s.setup?.direction==="BUY"?"bullish":s.setup?.direction==="SELL"?"bearish":null;
 const latest=s.events?.at(-1);
 const structure=setupDir&&latest&&recent(latest.index,20)
  ?(latest.strength==="displacement"&&recent(latest.index,10)?20:15)
  :0;

 const liquidity=setupDir
  ?s.sweeps?.some((x:any)=>recent(x.index,20)&&(
    (setupDir==="bullish"&&x.type==="low")||(setupDir==="bearish"&&x.type==="high")
   ))?15:0
  :0;

 const zones=setupDir
  ?(
    (s.fvgs?.some((x:any)=>x.type===setupDir&&!x.filled&&recent(x.to,20))?8:0)+
    (s.orderBlocks?.some((x:any)=>x.type===setupDir&&!x.mitigated&&recent(x.index,20))?12:0)
   )
  :0;

 const location=setupDir
  ?((setupDir==="bullish"&&s.premiumDiscount==="Discount")||(setupDir==="bearish"&&s.premiumDiscount==="Premium")?10:0)
  :0;

 const momentum=setupDir&&s.displacement>=.7?15:setupDir?5:0;
 const volume=setupDir?Math.min(10,Math.round(Math.max(0,flow.volumeRatio-0.7)*6)):0;
 const regimeBonus=setupDir&&regime.regime.startsWith("TRENDING")?5:0;
 const total=Math.max(0,Math.min(100,structure+liquidity+zones+location+momentum+volume+regimeBonus));
